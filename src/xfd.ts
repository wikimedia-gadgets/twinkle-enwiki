...
class Tfd extends XfdMode {
	...
	preprocessParams() {
		if (this.params.tfdtarget) {
			this.params.tfdtarget = stripNs(this.params.tfdtarget);
		}
	...
	notifyOtherCreator() {
		...
		return new Page(this.params.otherTemplateName, 'Finding other page creator').lookupCreation().then(page => {
	...
	getDiscussionWikitext(): string {
		return makeTemplate('subst:' + this.params.xfdcat + '2', {
...
class Ffd extends XfdMode {
	...
	addToList() {
		let params = this.params;
		var wikipedia_page = new Page(params.logpage, "Adding discussion to today's list");
...
class Cfd extends XfdMode {
	...
	preprocessParams() {
		if (this.params.cfdtarget) {
			this.params.cfdtarget = stripNs(this.params.cfdtarget);
		}
		if (this.params.cfdtarget2) {
			this.params.cfdtarget2 = stripNs(this.params.cfdtarget2);
		}
