...
export class Arv extends TwinkleModule {
	...
	constructor() {
		...
		Twinkle.addPortletLink(
			...
			'ARV',
			'tw-arv',
			'Report ' + userType + ' to administrators'
		);
	}

	static callback(uid, isIP) {
		...
		new Morebits.wiki.user(uid, "Checking the user's block status").load(function (userobj) {
			if (userobj.isBlocked()) {
				var message =
					(isIP ? 'This IP ' + (userobj.isIPRange() ? 'range' : 'address') : 'This account') +
					' is ' +
					(userobj.getPartial() ? 'partially' : 'already') +
					' blocked';
					...
					list: [
						{
							...
							value: 'misleading',
							tooltip:
								'Misleading usernames imply relevant, misleading things about the contributor. For example, misleading points of fact, an impression of undue authority, or usernames giving the impression of a bot account.',
						},
						{
					...
					list: [
						{
							...
							name: 'checkuser',
							tooltip:
								'CheckUser is a tool used to obtain technical evidence related to a sockpuppetry allegation. It will not be used without good cause, which you must clearly demonstrate. Make sure your evidence explains why using the tool is appropriate. It will not be used to publicly connect user accounts and IP addresses.',
						},
						{
		...
		switch (form.category.value) {
				...
				aivPage.load(function () {
					...
					if (
						...
						Morebits.status.printUserText(
							reason,
							'The comments you typed are provided below, in case you wish to manually post them under the existing report for this user at ' +
								$aivLink +
								':'
						);
						return;
					...
					tb2Page.load(function () {
						...
						if (
							...
							if (
								confirm(
									'The user ' + uid + ' has already been reported by a bot. Do you wish to make the report anyway?'
								)
							) {
								...
								Morebits.status.printUserText(
									reason,
									'The comments you typed are provided below, in case you wish to manually post them at ' +
										$aivLink +
										':'
								);
								return;
			...
			case 'username':
				...
				if (types.length || hasShared) {
					reason +=
						'Violation of the username policy as ' +
						article +
						' ' +
						types +
						' username' +
			...
			case 'puppet':
				...
				if (puppetReport && !form.sockmaster.value.trim()) {
					alert(
						'You have not entered a sockmaster account for this puppet. Consider reporting this account as a sockpuppeteer instead.'
					);
					return;
				} else if (!puppetReport && !form.sockpuppet[0].value.trim()) {
					alert(
						'You have not entered any sockpuppet account(s) for this sockmaster. Consider reporting this account as a sockpuppet instead.'
					);
					return;
				}
